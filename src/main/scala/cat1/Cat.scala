...
trait Cat[O, Arr[_ <: O, _ <: O]] {

  type Obj = O

  type Arrow[A <: O, B <: O] = Arr[A, B]
...
object Prod {
  ...
  trait Arr[T[_,_]] {
    def compose[A, B, C](g: T[B, C], f: T[A, B]): T[A, C]
  }

  sealed trait ParArr[A, B]

/*
  def prueba[A1, B1, A2, B2, Arr1[_,_]: Arr, Arr2[_,_]: Arr](
    a: Arr1[_,_], 
    b: Arr2[_,_]): Arr[ParArr] = new Arr[ParArr] {
      def compose[A, B, C](g: T[B, C], f: T[A, B]): T[A, C] = null
    }
*/
}


object Set extends Cat[Any, Function1] {

  def compose[A, B, C](
    g: B => C, 
    f: A => B): A => C = g compose f

  def identity[A]: A => A = Predef.identity[A]

}

/*
class Prod[CObj, CArr[_ <: CObj, _ <: CObj], DObj, DArr[_ <: DObj, _ <: DObj]](
  c: Cat[CObj, CArr], 
  d: Cat[DObj, DArr]) extends Cat[Pair[_ <: CObj, _ <: DObj], ({ type Arr[A <: Pair[_ <: CObj, _ <: DObj], B <: Pair[_ <: CObj, _ <: DObj]] = Prod.Arrow[A, B, CArr, DArr] })#Arr] {
